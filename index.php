<?php
 ...
 if('OPTIONS' == $_SERVER['REQUEST_METHOD']){
 	...
 	header("Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept, frm-session, x-proxy-url");
 	echo '';
 	exit;
 }
...
if ( 'POST' == $request_method ) {
    ...
    foreach ($_FILES as $f => $file) {
        if($file['size']){
            ...
            $file_params[$f] = curl_file_create($file['tmp_name'], $file['type'], $file['name']);
            $has_files = true;
        }
    }
